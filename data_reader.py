...
def person_from_dict(data, name, args, to_optimize_values):
    ...
    for v in to_optimize_values:
        ...
        p.values_optimal[v] = (
            data[v]["opt"] if (v in data and "opt" in data[v]) else args.opt_default
        )

        p.values_sensitivity[v] = (
            data[v]["sens"] if (v in data and "sens" in data[v]) else args.sens_default
        )
        p.fixed_values[v] = 0
    return p
...
def thing_from_dict(data, name, args, people, to_optimize_values):
    ...
    if "fixed" in data:
        t.fixed = (
            get_person_by_name(people, data["fixed"])
            if data["fixed"] is not True
            else t.owner
        )
...
def read_yaml(args):
    ...
    if "config" in data:
        for attribute in data["config"]:
            if help_parser.is_default(
                args, attribute
            ):  # command (args) has more priority
    ...
    data_classes.Thing.moral = args.moral_default
    ...
    if "optimize" in data:
        ...
        for v_name in data["optimize"]:
            ...
            if "pain" in data["optimize"][v_name]:
                v.pain = data["optimize"][v_name]["pain"]
    ...
    for person_name in data["people"]:
        try:
            people.append(
                person_from_dict(
                    data["people"][person_name], person_name, args, to_optimize_values
                )
            )
...
def thing_from_list(tlist, people, args):
    ...
    t.owner = None if tlist[2] == "None" else get_person_by_name(people, tlist[2])
    ...
    try:
        ...
        for line in open(people_file, encoding="utf-8"):
            ...
            if args.auto_complete:
                current[1:] = auto_complete(current[1:], [10, 10])
        ...
        for line in open(things_file, encoding="utf-8"):
            ...
            if not current:
                continue
            if args.auto_complete:
                current[1:] = auto_complete(current[1:], [1.0, None, 0.0])

            things.append(thing_from_list(current, people, args))

    except IndexError:
        raise SyntaxError(
            "Invalid file input length"
            + (
                ". Try -a option to automaticly add insufficient values."
